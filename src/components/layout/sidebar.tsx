...
import { LayoutDashboard, Users, FolderKanban, Receipt, CheckSquare, BarChart3, MessageSquare, Settings, ChevronLeft, Menu, MessageCircle, UsersRound } from "lucide-react";
            ...
            return (
              <Link key={item.href} href={item.href}
                className={cn("flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all",
                  isActive ? "bg-brand-green/10 text-brand-green" : "text-gray-400 hover:text-white hover:bg-white/5")}>
          ...
          <div className="p-4 border-t border-white/5">
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 bg-gradient-to-br from-brand-green/20 to-brand-blue/20 rounded-full flex items-center justify-center">
                <span className="text-xs font-semibold text-brand-green">{user.name.charAt(0).toUpperCase()}</span>
              </div>
