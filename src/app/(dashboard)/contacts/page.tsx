...
import { Plus, Search, Users, X, Phone, Mail, Building, Tag, Trash2, Edit } from "lucide-react";
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 bg-brand-coral hover:bg-brand-coral/90 text-white px-4 py-2 rounded-lg text-sm font-semibold transition"
        >
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-surface border border-white/5 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-brand-coral/30 transition"
        />
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={form.full_name} onChange={(e) => setForm({...form, full_name: e.target.value})}
                  className="w-full bg-brand-navy border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-brand-coral/50 transition" required />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Email</label>
                  <input type="email" value={form.email} onChange={(e) => setForm({...form, email: e.target.value})}
                    className="w-full bg-brand-navy border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-brand-coral/50 transition" />
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="text" value={form.tags} onChange={(e) => setForm({...form, tags: e.target.value})}
                    ...
                    className="w-full bg-brand-navy border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-brand-coral/50 transition" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">Notes</label>
                <textarea value={form.notes} onChange={(e) => setForm({...form, notes: e.target.value})} rows={3}
                  className="w-full bg-brand-navy border border-white/10 rounded-lg px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-brand-coral/50 transition resize-none" />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit"
                  className="flex-1 bg-brand-coral hover:bg-brand-coral/90 text-white py-2.5 rounded-lg text-sm font-semibold transition">
      ...
      {loading ? (
        <div className="bg-surface border border-white/5 rounded-xl p-12 text-center">
          <div className="w-6 h-6 border-2 border-brand-coral border-t-transparent rounded-full animate-spin mx-auto" />
        </div>
      ) : contacts.length === 0 ? (
        <div className="bg-surface border border-white/5 rounded-xl p-12 text-center">
          ...
          <div className="divide-y divide-white/5">
            {contacts.map((contact) => (
              <div key={contact.id} className="flex items-center gap-4 px-5 py-4 hover:bg-white/[0.02] transition group">
                {/* Avatar */}
                <div className="w-10 h-10 bg-brand-navy-light rounded-full flex items-center justify-center flex-shrink-0">
                  ...
                  {contact.tags && contact.tags.length > 0 && (
                    <div className="flex gap-1.5 mt-1.5">
                      {contact.tags.map((tag) => (
                        <span key={tag} className="text-[10px] bg-brand-coral/10 text-brand-coral px-2 py-0.5 rounded-full">
                          {tag}
                        </span>
                      ))}
                    </div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-600 hidden sm:block">{formatRelative(contact.created_at)}</span>
